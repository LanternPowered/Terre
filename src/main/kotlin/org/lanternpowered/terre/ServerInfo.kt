 ...
 *
 * If [protocolVersion] is null, the proxy will attempt to connect with
 * multiple versions. This has the benefit that you can swap the server
 * without having to modify the info, but the first time someone connects
 * to the server can take a bit longer. After the server has discovered
 * the version there shouldn't be a difference in connecting time.
 *
